...
Description: This program aims to parallelize the Lowest Common Ancestor algorithm

**/

__global__ void parallelMerge(int* dest_list, int dest_list_length, int* arrayLeft, int arrayLeft_length, int* arrayRight){

	int index = blockIdx.x * blockDim.x+ threadIdx.x + 1;
	int stride = blockDim.x * gridDim.x;
	int arrayRightIndex = 0;

	for(int i = index; i <= dest_list_length; i+= stride){

		if(i <= arrayLeft_length){
			dest_list[i] = arrayLeft[i];
		}
		else{
			arrayRightIndex = i - arrayLeft_length;
			int minNumLeft = arrayLeft[arrayLeft_length];
			int arrayRightNum = arrayRight[arrayRightIndex];

			if(minNumLeft <= arrayRightNum){
				dest_list[i] = minNumLeft;
			}else{
				dest_list[i] = arrayRightNum;
			}
		}
		__syncthreads();
	...
	for(int i = height-2; i>=0; i--){
		for(int x = 1; x <= pow(2,i);x++){
			...
			*/
			...
			//for prefix tree
			int dest_length_host = numOfChildren(index,numInputs);
			int arrayLeft_length_host = numOfChildren(left_index,numInputs);

			//int *destinationHost = prefixTree[index];
			//int *arrayLeft_host = prefixTree[left_index];
			//int *arrayRight_host = prefixTree[right_index];

			int *destinationDevice;
			int *arrayLeft_device;
			int *arrayRight_device;

			cudaMalloc((void**) &destinationDevice, (dest_length_host+1) * sizeof(int));
			cudaMalloc((void**) &arrayLeft_device, (arrayLeft_length_host+1) * sizeof(int));
			cudaMalloc((void**) &arrayRight_device, (arrayLeft_length_host+1) * sizeof(int));

			cudaMemcpy(arrayLeft_device, prefixTree[left_index], 
				(arrayLeft_length_host+1) * sizeof(int), cudaMemcpyHostToDevice);
			cudaMemcpy(arrayRight_device, prefixTree[right_index], 
				(arrayLeft_length_host+1) * sizeof(int), cudaMemcpyHostToDevice);

			int threads_per_block = 512;
			int numBlocks = (dest_length_host + threads_per_block - 1) / threads_per_block;
			...
			parallelMerge<<<numBlocks,threads_per_block>>>(destinationDevice, dest_length_host, 
				arrayLeft_device, arrayLeft_length_host, arrayRight_device);
			...
			cudaMemcpy(prefixTree[index], destinationDevice, 
				(dest_length_host+1) * sizeof(int), cudaMemcpyDeviceToHost);
			
			//for suffix tree
			cudaMemcpy(arrayLeft_device, suffixTree[left_index], 
				(arrayLeft_length_host+1) * sizeof(int), cudaMemcpyHostToDevice);
			cudaMemcpy(arrayRight_device, suffixTree[right_index], 
				(arrayLeft_length_host+1) * sizeof(int), cudaMemcpyHostToDevice);

			parallelMerge<<<numBlocks,threads_per_block>>>(destinationDevice, dest_length_host, 
				arrayRight_device, arrayLeft_length_host, arrayLeft_device);

			cudaMemcpy(suffixTree[index], destinationDevice, 
				(dest_length_host+1) * sizeof(int), cudaMemcpyDeviceToHost);
		}
	}
}
